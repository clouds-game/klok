...
from commandline_printing import (
...
def save_note_events(
  ...
  with open(save_path, "w") as fhandle:
    ...
    for start_time, end_time, note_number, amplitude, pitch_bend in note_events:
      row = [start_time, end_time, note_number, int(
          np.round(DEFAULT_MIDI_VELOCITY_SCALE * amplitude))]
      if pitch_bend:
...
# audio_path = WORKSPACE / "res/vocadito_10.wav"

# output: Dict[str, Any] = {"note": [], "onset": [], "contour": []}
# for audio_windowed, audio_window_time, audio_original_length in get_audio_input(audio_path, overlap_len, hop_size):
#   print(audio_windowed.shape, audio_window_time, audio_original_length)


# %%
# audio_path = WORKSPACE / "res/vocadito_10.wav"
def predict_and_save_midi(audio_path: pathlib.Path, midi_path: pathlib.Path):
...
predict_and_save_midi(audio_path, midi_path)
